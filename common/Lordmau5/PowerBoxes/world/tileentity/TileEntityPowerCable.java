...
public class TileEntityPowerCable extends TileEntity implements IEnergyStorage, IEnergySink, IPowerReceptor {
        ...
        for(int i=0; i<6; i++) {
            ...
            if(tile != null) {
                ...
                if(tile instanceof TileEntityPowerCable) {
                    ...
                    connect = true;
                }else if(PowerBoxes.isValidPowerTile(tile)) {
    ...
    public boolean acceptsEnergyFrom(TileEntity emitter, ForgeDirection direction) {
        if(emitter instanceof TileEntityPowerCable)
            return false;
        if(!PowerBoxes.isValidPowerTile(emitter))
            return false;
    ...
    public void convertBC() {
        ...
        network.addPower((int) Math.floor(PowerBoxes.bcComp.getPowerHandler(this).useEnergy(1, PowerBoxes.bcComp.getPowerHandler(this).getMaxEnergyStored(), true) / Ratios.MJ.conversion));
    }
    ...
    public void tryOutputtingEnergy() {
        ...
        for(int i=0; i<6; i++) {
            ...
            if(tmpTile != null) {
                if(!PowerBoxes.isInvalidPowerTile(tmpTile) && !(tmpTile instanceof TileEntityPowerCable)) {
        ...
        for(int i=0; i<6; i++) {
            if(conSides[i] != false) {
                ...
                if(tmptile instanceof IPowerReceptor && PowerBoxes.isValidPowerTile(tmptile) && !(tmptile instanceof TileEntityPowerCable)) {
    ...
    public PowerHandler.PowerReceiver getPowerReceiver(ForgeDirection side) {
        ...
        if(tmpTile != null && PowerBoxes.isValidPowerTile(tmpTile))
