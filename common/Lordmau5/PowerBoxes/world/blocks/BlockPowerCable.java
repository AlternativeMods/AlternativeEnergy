...
public class BlockPowerCable extends BlockContainer {
    ...
    {
        this.blockIcon = par1IconRegister.registerIcon(PowerBoxes.modid + ":" + this.getUnlocalizedName().substring(5));
    }
    ...
    public void onNeighborTileChange(World world, int x, int y, int z, int tileX, int tileY, int tileZ)
    {
        ...
        if(xTile == null || PowerBoxes.isInvalidPowerTile(xTile))
            me.getEnergyNetwork().removeInput(me, xTile);
        else
            if(PowerBoxes.isInvalidPowerTile(xTile))
    ...
    public void setCableBoundingBox(IBlockAccess bAccess, int x, int y, int z)
    {
        ...
        if(xTile != null)
        {
            if(PowerBoxes.isValidPowerTile(xTile))
                minX = 0.0F;
        }
